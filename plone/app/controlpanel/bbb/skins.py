...
class SkinsControlPanelAdapter(object):
    ...
    def __init__(self, context):
        super(SkinsControlPanelAdapter, self).__init__()
    ...
    def set_mark_special_links(self, value):
        ...
        if self.props.hasProperty('mark_special_links'):
            self.props.manage_changeProperties(
    ...
    mark_special_links = property(
        get_mark_special_links,
    ...
    def get_use_popups(self):
        ...
        return self.csstool\
            .getResource('++resource++plone.app.jquerytools.overlays.css')\
            .getEnabled()
...
def syncPloneAppRegistryToSkinsProperties(settings, event):
    portal = getSite()
    skin_properties = getAdapter(portal, ISkinsSchema)

    if event.record.fieldName == "theme":
        skin_properties.set_theme(settings.theme)
        return

    if event.record.fieldName == "mark_special_links":
        skin_properties.set_mark_special_links(settings.mark_special_links)
        return

    if event.record.fieldName == "ext_links_open_new_window":
        skin_properties.set_ext_links_open_new_window(
            settings.ext_links_open_new_window)
        return

    if event.record.fieldName == "icon_visibility":
        skin_properties.set_icon_visibility(
